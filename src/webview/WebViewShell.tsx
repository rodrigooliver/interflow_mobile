...
export interface WebViewShellProps {
  /** Sessão nativa a hidratar no WebView quando ready */
  sessionPayload?: AuthSessionPayload | null;
  /** Chamado quando a web envia auth.session / auth.refreshed */
  onRemoteSession?: (payload: AuthSessionPayload) => void;
  /** Chamado quando a web envia auth.logout */
  onRemoteLogout?: () => void;
  /** Último evento de auth nativo — dispara hydrate/signOut no WebView */
  nativeAuthEvent?: 'SIGNED_IN' | 'SIGNED_OUT' | 'TOKEN_REFRESHED' | 'INITIAL' | null;
  /** Se false, WebView fica aquecido offscreen (opacity 0) */
  visible?: boolean;
...
const WebViewShell = ({
  ...
  const hydrateWebViewSession = (payload?: AuthSessionPayload | null) => {
    ...
    if (!next?.access_token || !webViewRef.current) return;
    webViewRef.current.injectJavaScript(buildHydrateInjectScript(next));
  };

  useEffect(() => {
    if (!nativeAuthEvent || !webViewRef.current) return;
    if (nativeAuthEvent === 'SIGNED_OUT') {
      webViewRef.current.injectJavaScript(buildSignOutInjectScript());
      return;
    }
    if (
      nativeAuthEvent === 'SIGNED_IN' ||
      nativeAuthEvent === 'TOKEN_REFRESHED'
    ) {
  ...
  const processNotification = async (event: NotificationEvent) => {
    try {
      ...
      if (event.notification.additionalData) {
        ...
        const candidatePath = additionalData.path || additionalData.url || '';
        if (candidatePath && onNativeChatNavigate && isChatPath(candidatePath)) {
          const handled = onNativeChatNavigate(
            extractChatIdFromPath(candidatePath),
            candidatePath,
          );
          if (handled) return;
        }
  ...
  const handleWebViewMessage = (event: WebViewMessageEvent) => {
    try {
      ...
      if (data.type === 'linkClicked' && Platform.OS === 'android') {
        ...
        (data.type === 'auth.session' || data.type === 'auth.refreshed') &&
        data.access_token &&
        data.refresh_token
      ) {
        onRemoteSession?.({
        ...
        // OneSignal.login fica no AuthContext / sessionMirror (dedupe por user id)
      }
      // Verificar se é uma mensagem de login
      else if (data.type === 'login' && data.userId) {
        // Login OneSignal tratado no espelhamento de sessão (evita spam)
      }
  ...
  const handleLoadEnd = () => {
    try {
      ...
      Sentry.addBreadcrumb({
        category: 'webview',
        message: 'webview_load_end',
        data: {
          webview_ready_ms: webViewReadyAtRef.current - bootStartedAtRef.current,
          visible,
        },
      });

      // Esconder a splash screen nativa agora que o WebView está pronto
      // Em modo warm (hybrid), o shell nativo controla o splash
      try {
        if (visible && SplashScreen.hide) {
          SplashScreen.hide();
        }
      } catch (splashError) {
      ...
      try {
        ...
        window.__nativeAuth = window.__nativeAuth || {
          hydrate: function(payload) {
            try {
              ...
              var evt = new CustomEvent('nativeAuthHydrate', { detail: payload });
              window.dispatchEvent(evt);
            } catch (e) {
              console.error('[__nativeAuth.hydrate]', e);
            }
          },
          signOut: function() {
            try {
              window.__pendingNativeSession = null;
              var evt = new CustomEvent('nativeAuthSignOut');
              window.dispatchEvent(evt);
            } catch (e) {
              console.error('[__nativeAuth.signOut]', e);
            }
          }
        };
        ...
        style.innerHTML = \`
          ...
          * {
          ...
          /* Não forçar top em todos os <nav> — a bottom bar do web usa role=navigation
             e qualquer seletor amplo em nav quebra fixed bottom-0 (estica full height). */
          .fixed-header, .sticky-top, header {
            top: 0 !important;
          }

          .fixed-footer, footer, .mobile-nav-bar {
            bottom: 0 !important;
            top: auto !important;
          }
    ...
    <SafeAreaProvider>
      ...
      <SafeAreaView
        style={[
          styles.container,
          {backgroundColor: currentBackground},
          !visible && styles.warmHidden,
        ]}
        edges={visible ? ['top', 'bottom', 'left', 'right'] : []}
        ...
        >
          ...
          <WebView
            ...
            style={[
              ...
              (!isWebViewVisible || !visible) && styles.webviewHidden
            ]}
